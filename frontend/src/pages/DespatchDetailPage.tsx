...
import { DespatchPreview, openDespatchPrintWindow } from '../components/preview/DespatchPreview'
...
export default function DespatchDetailPage() {
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				setLoading(true)
				const res = await api.get(`/despatches/${id}`)
	...
	async function cancel() {
		if (!id) return
		const ok = await confirm({ title: 'İptal', description: `İrsaliye #${id} iptal edilsin mi?`, variant: 'danger', confirmText: 'İptal Et' })
		if (!ok) return
		setBusy(true)
		try {
			const res = await api.post(`/despatches/${id}/cancel`)
			setData(res.data)
	...
	async function retry() {
		if (!id) return
		const ok = await confirm({ title: 'Yeniden dene', description: `İrsaliye #${id} yeniden denensin mi?` })
		if (!ok) return
		setBusy(true)
		try {
			const res = await api.post(`/despatches/${id}/retry`)
			setData(res.data)
			...
			{!loading && data && (
				<div style={{ display: 'grid', gap: 12 }}>
					...
					<div style={{ display: 'flex', gap: 8 }}>
						...
						<button onClick={() => openDespatchPrintWindow(data)}>Yazdır</button>
					</div>
					<div>
						<h3>Önizleme</h3>
						<DespatchPreview data={data} />
					</div>
					{data.xml && (
						<div>
							<h3>XML</h3>
