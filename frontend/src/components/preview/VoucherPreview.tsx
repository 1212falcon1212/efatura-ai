export function VoucherPreview({ data }: { data: any }) {
  return (
    <div style={{ fontFamily: 'ui-sans-serif, system-ui', background: 'white', color: '#0f172a', border: '1px solid #e2e8f0', borderRadius: 12, padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          ...
          <div style={{ color: '#64748b' }}>E‑MAKBUZ</div>
        </div>
...
export function openVoucherPrintWindow(data: any) {
  ...
  const html = `<div style="font-family: ui-sans-serif, system-ui; background: white; color: #0f172a; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px;">
    <div style="display:flex;justify-content:space-between;margin-bottom:12px">
      <div><div style="font-weight:700;font-size:18px">efatura.ai</div><div style="color:#64748b">E‑MAKBUZ</div></div>
      <div style="text-align:right"><div><strong>No:</strong> ${data?.id ?? '-'}</div><div><strong>Tip:</strong> ${data?.type ?? '-'}</div></div>
    </div>
    <div><strong>ETTN:</strong> ${data?.ettn ?? '-'}</div>
  </div>`
  win.document.write(`<html><head><title>Makbuz #${data?.id ?? ''}</title><style>${style}</style></head><body>${html}</body></html>`)
